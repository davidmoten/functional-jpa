...
public class RichEntityManagerFactory {
    ...
    private static org.slf4j.Logger log = LoggerFactory.getLogger(RichEntityManagerFactory.class);
     ...
     * @return
     */
    public <T> TaskOptionalResult<T> run(Task<T> task, boolean throwException, boolean logException) {
    ...
    public RichEntityManagerFactory run(final TaskVoid task, boolean throwException,
            boolean logError) {
        run(new Task<Void>() {
            @Override
    ...
    public RichEntityManagerFactory runScript(InputStream is, Charset charset) {
        ...
        List<String> commands = from(newArrayList(items)).filter(notNull()).toList();
        run(commands);
        return this;
    }
            ...
            public void run(RichEntityManager em) {
                for (String command : commands) {
                    ...
                    if (command.trim().length() > 0)
                        em.get().createNativeQuery(command.trim()).executeUpdate();
                }
            }
        });
    }
