...
public class RichEntityManagerFactoryTest {
    ...
    public void testRichEmf() {
        ...
        ImmutableList<String> list = em.begin().persist(new Document("a"))
                ...
                .createQuery("from Document order by id", Document.class).fluent().transform(toId)
                .toList();
    ...
    public void testRichEmfRun() {
        ...
        emf.run(new TaskVoid() {
            ...
            public void run(RichEntityManager em) {
                ImmutableList<String> list = em.persist(new Document("a"))
                        .persist(new Document("b")).persist(new Document("c"))
                        .createQuery("from Document order by id", Document.class).fluent()
    ...
    public void testRichEmfRunReturningList() {
        ...
        List<String> list = emf.run(new Task<List<String>>() {
            ...
            public List<String> run(RichEntityManager em) {
                return em.persist(new Document("a")).persist(new Document("b"))
    ...
    public void testRichEmfRunReturningListAndUsingProcessor() {
        ...
        emf.run(new Task<List<String>>() {
            @Override
            public List<String> run(RichEntityManager em) {
                return em.persist(new Document("a"), new Document("b"), new Document("c"))
                        .createQuery("from Document order by id", Document.class).fluent()
                        .transform(toId).toList();
            }
    ...
    public void testCreateEntityManager() {
        emf("test").createEntityManager(Maps.<String, Object> newHashMap()).begin()
                .persist(new Document("a")).commit().close();
    }
