...
public class TypedQueryIteratorTest {
    ...
    public void testEntityManagerUsesParameter() {
        ...
        EntityManager em = emf.createEntityManager();
        insertDocuments(em);
        TypedQuery<Document> q = em.createQuery("from Document where id <> :exclude order by id",
                Document.class);
        assertEquals(newArrayList("a", "c"),
                query(q).parameter("exclude", "b").fluent().transform(toId).toList());
        emf.close();
    }
    ...
    public void testIteratorReturnsAllDocumentsUsingTwoPages() {
        ...
        insertDocuments(em);
        TypedQuery<Document> q = em.createQuery("from Document order by id", Document.class);
        assertEquals(newArrayList("a", "b", "c"), query(q).pageSize(2).fluent().transform(toId)
                .toList());
        emf.close();
    }
    ...
    public void testIteratorReturnsAllDocumentsUsingThreePages() {
        ...
        insertDocuments(em);
        TypedQuery<Document> q = em.createQuery("from Document order by id", Document.class);
        assertEquals(newArrayList("a", "b", "c"), query(q).pageSize(1).fluent().transform(toId)
                .toList());
        emf.close();
    }
